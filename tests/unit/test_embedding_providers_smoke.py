...
class TestOpenAIEmbedding:
    ...
    @patch('openai.OpenAI')
    def test_embed_retries_with_smaller_batches_on_batch_limit_error(
        self, mock_openai_class: Mock, mock_settings_openai: Any
    ) -> None:
        """Test provider batch-size errors are retried with smaller sub-batches."""
        mock_client = Mock()

        def create_embeddings(**kwargs: Any) -> Mock:
            inputs = kwargs["input"]
            if len(inputs) > 2:
                raise Exception("Error code: 413 - maximum allowed batch size 2")

            response = Mock()
            response.data = [Mock(embedding=[float(index)]) for index, _ in enumerate(inputs)]
            return response
        ...
        assert len(result) == 5
        batch_sizes = [len(call.kwargs["input"]) for call in mock_client.embeddings.create.call_args_list]
        assert batch_sizes == [5, 2, 3, 1, 2]


    @patch('openai.OpenAI')
    def test_embed_splits_single_input_when_token_limit_error(
        self, mock_openai_class: Mock, mock_settings_openai: Any
    ) -> None:
        """Test long single inputs are split and re-embedded recursively."""
        mock_client = Mock()

        def create_embeddings(**kwargs: Any) -> Mock:
            inputs = kwargs["input"]
            text = inputs[0]
            if len(inputs) == 1 and len(text) > 10:
                raise Exception("Error code: 413 - input must have less than 512 tokens")

            response = Mock()
            response.data = [Mock(embedding=[float(len(item)), 1.0]) for item in inputs]
            return response
        ...
        embedding = OpenAIEmbedding(mock_settings_openai, api_key="test-key")
        result = embedding.embed(["a very long input for embedding"])

        assert len(result) == 1
        assert len(result[0]) == 2
        call_lengths = [len(call.kwargs["input"][0]) for call in mock_client.embeddings.create.call_args_list]
        assert call_lengths[0] > 10
        assert min(call_lengths) <= 10
        assert call_lengths[-1] <= 10
