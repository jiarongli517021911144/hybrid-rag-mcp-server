...
def test_upsert_preserves_existing_chunk_id(upserter_with_mock_store, sample_chunk, sample_vector):
    """Upsert should preserve business-layer chunk IDs for cross-store consistency."""
    upserter, mock_store = upserter_with_mock_store
    sample_chunk.id = "doc_abc123_0000_deadbeef"

    returned_ids = upserter.upsert([sample_chunk], [sample_vector])

    assert returned_ids == ["doc_abc123_0000_deadbeef"]
    record = mock_store.upsert.call_args.args[0][0]
    assert record["id"] == "doc_abc123_0000_deadbeef"
    assert record["metadata"]["chunk_id"] == "doc_abc123_0000_deadbeef"
