...
class TestPipelineProgressCallback:
    ...
    def test_batch_failures_return_unsuccessful_result(self) -> None:
        fp = _make_fake_pipeline()
        fp.batch_processor.process.return_value.failed_chunks = 3
        fp.batch_processor.process.return_value.successful_chunks = 0
        fp.batch_processor.process.return_value.dense_vectors = []
        fp.batch_processor.process.return_value.sparse_stats = []

        result = IngestionPipeline.run(fp, "test.pdf", on_progress=None)

        assert not result.success
        assert "Encoding failed for 3 of 3 chunks" in result.error
        fp.integrity_checker.mark_failed.assert_called_once()
        fp.integrity_checker.mark_success.assert_not_called()
